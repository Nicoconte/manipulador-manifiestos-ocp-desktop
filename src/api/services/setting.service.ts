...
export const SettingService = {
    getGlobalSetting: async(): Promise<GlobalSetting> => {
        try {
            return (await pb.collection(PocketbaseCollections.Settings).getFullList<GlobalSetting>())[0];
        } catch(err) {
    ...
    updateTheme: async(theme:string) => {
        try {
            let setting = (await pb.collection(PocketbaseCollections.Settings).getFullList<GlobalSetting>())[0];
